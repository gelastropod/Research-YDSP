...
typedef enum {
    ...
    BENCH_CTR_AES,
    BENCH_CTR_BLAKE3,
    BENCH_CTR_BLAKE2S_256,
    BENCH_CTR_SHA3_256,
    BENCH_CTR_SHA256,
    BENCH_CBC_AES,
    BENCH_CBC_BLAKE3,
    BENCH_CBC_BLAKE2S_256,
    BENCH_CBC_SHA3_256,
    BENCH_CBC_SHA256,
    BENCH_IMPL_COUNT
} bench_impl;
...
static long parallel_max_threads_used = 0;
...
static fde_scheme implementation_scheme(bench_impl implementation) {
    ...
    case BENCH_CTR_BLAKE2S_256:
        return FDE_RATCHET_CTR_BLAKE2S_256;
    case BENCH_CTR_SHA3_256:
        return FDE_RATCHET_CTR_SHA3_256;
    case BENCH_CTR_SHA256:
        return FDE_RATCHET_CTR_SHA256;
    case BENCH_CBC_AES:
        return FDE_RATCHET_CBC_AES;
    case BENCH_CBC_BLAKE3:
        return FDE_RATCHET_CBC_BLAKE3;
    case BENCH_CBC_BLAKE2S_256:
        return FDE_RATCHET_CBC_BLAKE2S_256;
    case BENCH_CBC_SHA3_256:
        return FDE_RATCHET_CBC_SHA3_256;
    case BENCH_CBC_SHA256:
        return FDE_RATCHET_CBC_SHA256;
    default:
        return FDE_ECB;
    }
}
...
static int benchmark_throughput(const char *directory, const fde_keys *keys,
    ...
    fprintf(file,
            ...
            "mean_ns_per_sector,mean_ns_per_byte,median_ns_per_byte,"
            "mean_tsc_cycles_per_byte\n");
            ...
            for (int implementation = 0; implementation < BENCH_IMPL_COUNT;
                ...
                for (int operation = 0; operation < 2; ++operation) {
                    ...
                    fprintf(file,
                            ...
                            ns.mean / sectors,
                            ns.mean / (double)volume_bytes,
                            ns.median / (double)volume_bytes,
...
static int benchmark_ratchet_primitives(const char *directory,
    ...
    uint8_t hash_state[FDE_HASH_STATE_BYTES];
    ...
    for (int hash = 0; hash < FDE_HASH_COUNT; ++hash) {
        fill_data(hash_state, sizeof(hash_state), (uint32_t)(93 + hash));
        cycle_start = read_tsc();
        start = now_ns();
        for (size_t i = 0; i < iterations; ++i) {
            if (fde_ratchet_hash_update(worker, (fde_hash)hash, hash_state,
                                        ciphertext) != 0) {
                fclose(file);
                fde_worker_free(worker);
                return -1;
        ...
        fprintf(file, "%s(KconcatC),%zu,48,32,%llu,%.6f,%.6f\n",
                fde_hash_name((fde_hash)hash), iterations,
                (unsigned long long)elapsed,
                (double)elapsed / (double)iterations,
                FDE_HAS_TSC ? (double)cycles / (double)iterations : 0);
        benchmark_sink ^= hash_state[0];
    }
    benchmark_sink ^= aes_state[0];
    fclose(file);
    fde_worker_free(worker);
    return 0;
}
...
static int benchmark_parallel(const char *directory, const fde_keys *keys,
    ...
    long max_threads = online > 0 ? online : 1;
    const char *configured = getenv("FDE_MAX_THREADS");
    if (configured != NULL && configured[0] != '\0') {
        char *end = NULL;
        long requested = strtol(configured, &end, 10);
        if (end != configured && *end == '\0' && requested > 0 &&
    ...
    if (quick && max_threads > 4) {
        max_threads = 4;
    }
    parallel_max_threads_used = max_threads;
    ...
    fprintf(file,
            ...
            "stdev_ns,mean_mib_s,speedup,efficiency\n");
    ...
    for (int implementation = 0; implementation < BENCH_IMPL_COUNT;
        ...
        for (int threads = 1; threads <= max_threads; ++threads) {
            ...
            double mib_s = ((double)volume_bytes / (1024.0 * 1024.0)) /
                           (result.mean * 1e-9);
            fprintf(file, "%s,%d,%zu,%zu,%d,%.3f,%.3f,%.6f,%.6f,%.6f\n",
                    bench_name((bench_impl)implementation), threads,
                    sector_bytes, volume_bytes, trials, result.mean,
                    result.stdev, mib_s, speedup, speedup / (double)threads);
        }
    }
    fclose(file);
    free(input);
...
static int write_run_metadata(const char *directory, int quick) {
    ...
    fprintf(file, "parallel_max_threads=%ld\n", parallel_max_threads_used);
