...
struct fde_worker {
    ...
    EVP_CIPHER_CTX *hash_seed_left_enc;
    EVP_CIPHER_CTX *hash_seed_right_enc;
...
const char *fde_scheme_name(fde_scheme scheme) {
    static const char *const names[FDE_SCHEME_COUNT] = {
        ...
        "Ratchet-CTR-AES",
        "Ratchet-CTR-BLAKE3",
        "Ratchet-CTR-BLAKE2s-256",
        "Ratchet-CTR-SHA3-256",
        "Ratchet-CTR-SHA-256",
        "Ratchet-CBC-AES",
        "Ratchet-CBC-BLAKE3",
        "Ratchet-CBC-BLAKE2s-256",
        "Ratchet-CBC-SHA3-256",
        "Ratchet-CBC-SHA-256"
    };
...
const char *fde_hash_name(fde_hash hash) {
    static const char *const names[FDE_HASH_COUNT] = {
        "BLAKE3",
        "BLAKE2s-256",
        "SHA3-256",
        "SHA-256"
    };
    return (hash >= 0 && hash < FDE_HASH_COUNT) ? names[hash] : "unknown";
}

int fde_scheme_uses_indices(fde_scheme scheme) {
    return scheme == FDE_XTS ||
           (scheme >= FDE_RATCHET_CTR_AES &&
            scheme <= FDE_RATCHET_CTR_SHA256);
}
...
fde_worker *fde_worker_new(const fde_keys *keys) {
    ...
    worker->hash_seed_left_enc =
        new_cipher_ctx(EVP_aes_128_ecb(), keys->ratchet_k0, 1);
    worker->hash_seed_right_enc =
        new_cipher_ctx(EVP_aes_128_ecb(), keys->ratchet_k0 + 16, 1);
    ...
    worker->blake2s_ctx = EVP_MD_CTX_new();
    worker->sha3_ctx = EVP_MD_CTX_new();
    worker->sha256_ctx = EVP_MD_CTX_new();
    ...
    if (worker->ecb_enc == NULL || worker->ecb_dec == NULL ||
        ...
        worker->hash_seed_left_enc == NULL ||
        worker->hash_seed_right_enc == NULL ||
        ...
        worker->openssl_xts_dec == NULL || worker->blake2s_ctx == NULL ||
        worker->sha3_ctx == NULL || worker->sha256_ctx == NULL) {
...
void fde_worker_free(fde_worker *worker) {
    ...
    EVP_CIPHER_CTX_free(worker->hash_seed_left_enc);
    EVP_CIPHER_CTX_free(worker->hash_seed_right_enc);
    ...
    EVP_MD_CTX_free(worker->blake2s_ctx);
    EVP_MD_CTX_free(worker->sha3_ctx);
    EVP_MD_CTX_free(worker->sha256_ctx);
    free(worker);
}
...
/*
 * A hash ratchet needs a 256-bit AES data key from its first block onward.
 * Split the 256-bit K0 into two AES-128 keys and concatenate their outputs:
 * K_S,0 = AES-128(K0L, S) || AES-128(K0R, S).
 */
static int seed_hash_state(fde_worker *worker, uint64_t sector,
                           uint8_t state[FDE_HASH_STATE_BYTES]) {
    uint8_t encoded[16];
    encode_u64_le(sector, encoded);
    if (fixed_block(worker->hash_seed_left_enc, 1, encoded, state) != 0) {
        return -1;
    }
    return fixed_block(worker->hash_seed_right_enc, 1, encoded, state + 16);
}
...
static void blake3_update(uint8_t state[FDE_HASH_STATE_BYTES],
                          const uint8_t ciphertext[FDE_BLOCK_BYTES]) {
...
int fde_ratchet_hash_update(fde_worker *worker, fde_hash hash,
                            uint8_t state[FDE_HASH_STATE_BYTES],
                            const uint8_t ciphertext[FDE_BLOCK_BYTES]) {
    EVP_MD_CTX *ctx;
    const EVP_MD *digest;
    uint8_t input[FDE_HASH_STATE_BYTES + FDE_BLOCK_BYTES];
    unsigned int output_bytes = 0;

    if (worker == NULL || state == NULL || ciphertext == NULL || hash < 0 ||
        hash >= FDE_HASH_COUNT) {
        return -1;
    }
    if (hash == FDE_HASH_BLAKE3) {
        blake3_update(state, ciphertext);
        return 0;
    }

    switch (hash) {
    case FDE_HASH_BLAKE2S_256:
        ctx = worker->blake2s_ctx;
    ...
    memcpy(input, state, FDE_HASH_STATE_BYTES);
    memcpy(input + FDE_HASH_STATE_BYTES, ciphertext, FDE_BLOCK_BYTES);
    if (EVP_DigestInit_ex(ctx, digest, NULL) != 1 ||
        EVP_DigestUpdate(ctx, input, sizeof(input)) != 1 ||
        EVP_DigestFinal_ex(ctx, state, &output_bytes) != 1) {
        return -1;
    }
    return output_bytes == FDE_HASH_STATE_BYTES ? 0 : -1;
}
...
static int hash_scheme_parameters(fde_scheme scheme, int *is_ctr,
                                  fde_hash *hash) {
    switch (scheme) {
    case FDE_RATCHET_CTR_BLAKE3:
        *is_ctr = 1;
        *hash = FDE_HASH_BLAKE3;
        return 0;
    case FDE_RATCHET_CTR_BLAKE2S_256:
        *is_ctr = 1;
        *hash = FDE_HASH_BLAKE2S_256;
        return 0;
    case FDE_RATCHET_CTR_SHA3_256:
        *is_ctr = 1;
        *hash = FDE_HASH_SHA3_256;
        return 0;
    case FDE_RATCHET_CTR_SHA256:
        *is_ctr = 1;
        *hash = FDE_HASH_SHA256;
        return 0;
    case FDE_RATCHET_CBC_BLAKE3:
        *is_ctr = 0;
        *hash = FDE_HASH_BLAKE3;
        return 0;
    case FDE_RATCHET_CBC_BLAKE2S_256:
        *is_ctr = 0;
        *hash = FDE_HASH_BLAKE2S_256;
        return 0;
    case FDE_RATCHET_CBC_SHA3_256:
        *is_ctr = 0;
        *hash = FDE_HASH_SHA3_256;
        return 0;
    case FDE_RATCHET_CBC_SHA256:
        *is_ctr = 0;
        *hash = FDE_HASH_SHA256;
        return 0;
    default:
...
static int ratchet_hash_crypt(fde_worker *worker, fde_scheme scheme,
    ...
    fde_hash hash;
    uint64_t start = profile == NULL ? 0 : now_ns();
    if (hash_scheme_parameters(scheme, &is_ctr, &hash) != 0 ||
        seed_hash_state(worker, sector, state) != 0) {
        return -1;
    }
    if (profile != NULL) {
        profile_add(&profile->seed_ns, start);
    ...
    for (size_t j = 0; j < blocks; ++j) {
        ...
        start = profile == NULL ? 0 : now_ns();
        if (fde_ratchet_hash_update(worker, hash, state,
                                    encrypt ? destination : source) != 0) {
            return -1;
        }
        if (profile != NULL) {
...
int fde_encrypt(fde_worker *worker, fde_scheme scheme,
    ...
    case FDE_RATCHET_CBC_SHA3_256:
    case FDE_RATCHET_CBC_SHA256:
        return ratchet_hash_crypt(worker, scheme, 1, plaintext, indices,
                                  sector, blocks, ciphertext, profile);
    default:
        return -1;
    }
...
int fde_decrypt(fde_worker *worker, fde_scheme scheme,
    ...
    case FDE_RATCHET_CBC_SHA3_256:
    case FDE_RATCHET_CBC_SHA256:
        return ratchet_hash_crypt(worker, scheme, 0, ciphertext, indices,
                                  sector, blocks, plaintext, profile);
    default:
        return -1;
    }
...
int fde_decrypt_target(fde_worker *worker, fde_scheme scheme,
    ...
    }
    {
        ...
        for (size_t j = 0; j <= target; ++j) {
            ...
            if (j < target) {
                if (fde_ratchet_hash_update(worker, hash, state, block) != 0) {
                    return -1;
                }
            }
        }
